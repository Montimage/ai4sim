...
# Forcer le flush de la sortie pour un affichage en temps réel
sys.stdout.reconfigure(line_buffering=True)

# Redéfinir print pour forcer le flush automatiquement
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    def scan_target(self, target, mode="full"):
        ...
        # Résultats simulés
        # S'assurer qu'on a des exploits dans la base
        if not self.exploits_db:
            self.initialize_exploits_tree()
        
        # Sélectionner des vulnérabilités aléatoirement
        num_vulns = random.randint(1, min(3, len(self.exploits_db)))
        vulnerabilities = random.sample(self.exploits_db, num_vulns) if self.exploits_db else []
...
def main():
    ...
    # Utiliser parse_known_args pour ignorer les arguments non reconnus
    args, unknown = parser.parse_known_args()
    
    # Afficher les arguments ignorés si il y en a (pour debug)
    if unknown:
        print(f"⚠️  Ignoring unknown arguments: {' '.join(unknown)}")
