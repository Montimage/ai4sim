...
function App() {
  ...
  // Écouter les messages de plein écran du parent
  React.useEffect(() => {
    const handleMessage = (event) => {
      if (event.data && event.data.type === 'FULLSCREEN_CHANGE') {
        setIsFullscreen(event.data.isFullscreen);
        
        // Appliquer la classe CSS pour optimiser l'affichage en plein écran
        if (event.data.isFullscreen) {
          document.body.classList.add('fullscreen-optimized');
          document.documentElement.classList.add('fullscreen-optimized');
        } else {
          document.body.classList.remove('fullscreen-optimized');
          document.documentElement.classList.remove('fullscreen-optimized');
        }
      }
    };
